  ...
  return async (request: NextRequest, event: NextFetchEvent) => {
    ...
    if(_permissions){
      ...
      const ifUserHasPermission = (path:string)=>{
        path = path?.startsWith("/en")||path?.startsWith("/ar")? path.split("/")[3]:path.split("/")[2]
        if(path?.includes( "profile") || path===undefined){
          return true
        }
        console.log("path",path)
        if((path?.includes( "admins") || path?.includes( "doctors")|| path?.includes( "nurses") || path?.includes( "patients") || path?.includes( "pharmacies")) && _permissions?.includes("Users")){
           return true
        }

        if((path?.includes( "reservations")||path?.includes( "pharmacy-orders")|| path?.includes( "nurse-orders")) && _permissions?.includes("Orders")){
           return true
        }
        ...
        if((path?.includes("cancel-requests")) && _permissions?.includes("Cancel Requests")){
           return true
        }
        if((path?.includes( "notifications")) && _permissions?.includes("Notifications")){
           return true
        }
        ...
        if((path?.includes("settings")) && _permissions?.includes("General Settings")){
           return true
        }

        if((path?.includes("verification-requests")) && _permissions?.includes("Verification Requests")){
           return true
        }
        if((path?.includes("data-management")) && _permissions?.includes("Data Management")){
           return true
        }
      }
